...
|---|---|
| Arduino | Controls the cube, reads the physical controls, manages the HM-10, and performs Custom rendering locally. |
...
| Animation generation | Built-in and Custom animations produce cube frame data through the common display system. |
...
| Waiting states | The Custom waiting state must blank/hold the display instead of continuing the previous animation. |
...
| Display mode | The current firmware uses the buffered display path for built-in and Custom rendering. No separate Stream mode is implemented or part of the current command protocol. |
...
Important existing RAM structures include `displayBuffer`, the packed Custom expression program, and the Bluetooth receive buffer.
