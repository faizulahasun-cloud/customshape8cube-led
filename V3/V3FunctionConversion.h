...
// X,Y,Z are voxel coordinates 0..7; F is animation frame 0..49.
// The engine never drives the cube or allocates frame buffers.
namespace V3FunctionConversion {

static const uint16_t MAX_FUNCTION_LENGTH = 160;
static const uint8_t MAX_BYTECODE_LENGTH = 72;
...
inline bool receiveCharacter(char c) {
  ...
  if ((uint8_t)c < 0x20 || (uint8_t)c > 0x7E) { receiveError = true; functionValid = false; return false; }
  if (c == FUNCTION_END) {
  ...
  if (functionLength >= MAX_FUNCTION_LENGTH) { receiveError = true; functionValid = false; return false; }
...
inline bool parseNumber() {
  ...
  if (parsePosition < functionLength && functionBuffer[parsePosition] == '.') {
    parsePosition++;
    if (parsePosition >= functionLength || functionBuffer[parsePosition] < '0' || functionBuffer[parsePosition] > '9') { parseError = true; return false; }
    float place = 0.1f;
    while (parsePosition < functionLength) {
      char c = functionBuffer[parsePosition];
      if (c < '0' || c > '9') break;
...
inline bool parseIdentifier() {
  ...
  if (n == 1) {
    char q = upperAscii(functionBuffer[start]);
    if (q == 'X') return emit(OP_X);
    if (q == 'Y') return emit(OP_Y);
    if (q == 'Z') return emit(OP_Z);
    if (q == 'F') return emit(OP_F);
  }
...
inline bool parseComparison() { if (!parseAddition()) return false; skipSpaces(); if (parsePosition >= functionLength) return true; char a = functionBuffer[parsePosition]; char b = (parsePosition + 1 < functionLength) ? functionBuffer[parsePosition + 1] : '\0'; uint8_t op = OP_END; if (a == '<' && b == '=') op = OP_LE; else if (a == '>' && b == '=') op = OP_GE; else if (a == '=' && b == '=') op = OP_EQ; else if (a == '!' && b == '=') op = OP_NE; else if (a == '<') op = OP_LT; else if (a == '>') op = OP_GT; if (op == OP_END) return true; parsePosition += (b == '=' ? 2 : 1); if (!parseAddition() || !emit(op)) return false; return true; }
...
inline bool evaluate(uint8_t X, uint8_t Y, uint8_t Z, uint8_t F) {
  ...
  float stack[20]; uint8_t sp = 0;
  for (uint8_t i = 0; i < bytecodeLength; i++) {
    const Instruction &ins = bytecode[i];
    switch (ins.op) {
      case OP_END: return sp ? (stack[sp - 1] != 0.0f) : false;
      case OP_CONST: if (sp >= 20) return false; stack[sp++] = ins.value; break;
      case OP_X: if (sp >= 20) return false; stack[sp++] = X; break;
      case OP_Y: if (sp >= 20) return false; stack[sp++] = Y; break;
      case OP_Z: if (sp >= 20) return false; stack[sp++] = Z; break;
      case OP_F: if (sp >= 20) return false; stack[sp++] = F; break;
